...
# design frequencies
Fc = 200 # lower cut-off frequency [MHz]
...
# read calculated values from numpy binaries
wsim = np.load(DATA_FOLDER+'/s.npy')
gs = np.load(DATA_FOLDER+'/g.npy')
Zsim = np.load(DATA_FOLDER+'/z.npy')

vph = np.load(DATA_FOLDER+'/vph.npy')
epssim = np.square(np.divide(scipy.constants.c, vph))

g50 = np.load(DATA_FOLDER+'/g50.npy')
w50 = np.load(DATA_FOLDER+'/s50.npy')

g50_interp = interp1d(w50, g50)
...
coords = generate_slope(coords, g50_interp, 0.1, analytical=True)
