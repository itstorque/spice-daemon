...
%close all;
saveflag = 1
% load('C:\Users\Di Zhu\Dropbox (MIT)\QNN lab\paper under preparation\2018 tapred readout\layout\taper back calculation\sc_cpw_impedance_80pH_gap=3um.mat')

%load('/Users/dizhu/Dropbox (MIT)/QNN lab/JPL collaboration/taper readout detector for JPL/JPL surp tapered readout/v2/taper back calculation/sc_cpw_impedance_80pH_gap=3um.mat')
% load(fullfile('impedance calculations', 'NbN80pH_3um.mat'));
%load('CPW_NbN_Lk=80_STO_eps=2100_thick=100nm_gap_12u.mat')
w_list = w_list;
Zin_list = Z60pHg1.Z0_list;
eps_list = eEff60pHg1.eps_list;

% load(fullfile('impedance calculations', 'grounded_CPW_120nm_HSQ_cap_1um_gap_2um_offset.mat'));
% w_list = w_list*1e6;
% Zin_list = z0_list;
% eps_list = neff_list.^2;



% fname = 'C:\Users\Di Zhu\Dropbox (MIT)\QNN lab\Simulations\Sonnet simulation\microstrip\microstrip impedance sweep\microstrip_impedance_list_all.csv';
Zsim = Zin_list;
wsim = w_list;
epssim = eps_list;

bending_radius_factor = 3; %normally I use 3
gap = 1; %gap size for cpw
w1 = 0.1;
lmax = 5000; %maximum length for each turn (this is for bending the taper)
Zload=interp1(wsim, Zsim, w1);  % Load impedance, to matched (Ohms)
...
%0.587 is for 20 dB ripple (max reflection coefficient magnitude in the
%pass band shall not exceed 1% of rho0
%total_length = 0.587*3e8/f/(2*pi);
RdB = -20; % unit in dB,  Operating band ripple (dB)
...
%calculate the total length of the taper
total_length=A*lmd0/(2*pi)     % total length of the taperd line

N = 1000;
dlmd0 = total_length/N; %length per section
Tlen = dlmd0/lmd0; %section length in terms of wavelength

% N=1000;       % Number of transformer sections
...
figure
Zlist=bklop(Zo,Zload,N,RdB);   % Klopfenstein taper impedance list
%This generate a taper from 50 ohm to high impedance

% Return loss and smith chart plots
bplot(Zlist,Tlen,Fo,F1,F2);
% bdraw(Zlist,Tlen,Fo,F1,F2,'g-');
% bdraw(Zlist,Tlen,Fo,F1,F2,'b-');
% axis([0 F2 -50 0]);
...
% title('taper profile')
% % 
% %cut_point = cut_point(2:end);
% t = datetime('now','Format','yyyy-MM-dd');
% fname =[char(t) '_taper_profile_' num2str(Fo) 'MHz_gap=' num2str(gap) 'um_w1=' num2str(w1*1000) 'nm_lmax=' num2str(lmax) 'um']
% 
% if saveflag == 1
% 
% save(fname, 'x0', 'y0', 'x1', 'x2', 'y1', 'y2', 'x3', 'y3','x4','y4','cut_point');
% end
% % 
% % 
